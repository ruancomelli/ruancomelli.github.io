...
{% now_section(icon="📍", title="Where", anchor_id="where") %}
[Florianópolis, SC, Brazil](https://en.wikipedia.org/wiki/Florian%C3%B3polis).
{% end %}

{% now_section(icon="💻", title="Work", anchor_id="work") %}
...
{% now_section(icon="📚", title="Studies", anchor_id="studies") %}
- **Rust**:
...
{% now_section(icon="📝", title="Blogging", anchor_id="blogging") %}
...
{% now_section(icon="🐶", title="Personal", anchor_id="personal", empty=true) %}
{% end %}

{% now_section(icon="💭", title="Thoughts", anchor_id="thoughts", empty=true) %}
{% end %}
